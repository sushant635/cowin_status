    ...
    try:
        if request.method == 'POST':
          ...
          #  print(request.GET)
            ...
            # print(data)
            department1 = request.GET.get('department')
            branch1 = request.GET.get('branch')
           ...
           # print('data',data)
            ...
            print('@@@@@@@@@@',filtered_qs)
            company_wise_data = filtered_qs.qs.filter(company=company).values('employee_branch','employee_department')
            print(company_wise_data)
            # paginated_filtered_person = Paginator(company_wise_data.qs.filter(company=company),10)
            # print(company_wise_data.qs.filter(company=company))
                    ...
                    for i in beneficiaries:
                        ...
                        if beneficiary_reference_id == Beneficiary_Id:
                            ...
                            if emp_first_name.upper() == first_name.upper():
                                ...
                                messages.error(request,'Employee name does not match with Cowin name')
                                return redirect('user_profile')
                        else:
                            pass
                else:
...
def exportcsv(request):
    try:
        if request.method == 'POST':
            ...
            for i in data:
                print(i)
                dose1_data = i['does1_date']
                dose2_date = i ['does2_date']
                diff_dose1 = None
                diff_dose2 = None
                if dose1_data != None:
                    delta = today_date - dose1_data
                    diff_dose1 = delta.days
                    print(delta)
                    print(delta.days)
                if dose2_date != None:
...
def admin_profile(request):
    try:
        ...
        context = {'company':company,'available':available,'consumed':consumed,'purchase':purchase,'user':user,'password':password,'form':form,'last_day':last_day,'last_time':last_time,'last_purchase':last_purchase,'second_day':second_day,'second_time':second_time,'second_last_purchase':second_last_purchase,'last_consumed':last_consumed,'second_last_consumed':second_last_consumed,\
                'conlast_day':conlast_day,'consecondlast_day':consecondlast_day,'conlast_time':conlast_time,'consecondlast_time':consecondlast_time}
